TITLE 'Multi-Morph Resonator'     { the problem identification }
...
qdotvol = dot(Efield,J) !Joule's law
...
BOUNDARIES       { The domain definition }
	...
	START(0,0) 
	load(Temp) = qconv load(u)=0 load(v)=0 
	line to (Lx, 0)
	load(Temp) = qconv
	line to (Lx, hbot+hmid)
	load(Temp) = 0
	line to (Lx-Ltip, hbot+hmid)
	load(Temp) = qconv
	line to (Lx-Ltip, hbot)
	load(Temp) = qconv
	line to (0,hbot)
	value(Voltage) = 0 value(u) = 0 value(v) = 0 value(Temp) = 20
	LINE TO close
	...
	line to (Lx, hbot+hmid)
	load(Temp) = qconv
	line to (Lx, hbot+hmid+htop)
	load(Temp) = qconv
	line to (0, hbot+hmid+htop)
	value(Voltage) = Vmax value(u) = 0 value(v) = 0 value(Temp) = 20
	LINE TO close
